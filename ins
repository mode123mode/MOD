...
¦ NOW you can not install the SOURCE MOD. \n
...
GET_INSTALL=$(redis-cli get MOD_INSTALL)
fi
if [ "${GET_INSTALL}" == "Yes" ]; then
echo -e "
${SMAY}>> ${LSAMAY}Source MOD${SMAY} is Already Installed ^_^ .\n${CNIL}"
sudo chmod +x ./run
sudo chmod +x ./TG
sudo ./run
exit;
...
wget "MOD.com/installubuntu18/compat-libevent2-5_2.0.21-1ubuntu18_amd64.deb" && sudo dpkg -i compat-libevent2-5_2.0.21-1ubuntu18_amd64.deb
...
cd && cd MOD && sudo find . -type f -print0 | xargs -0 chmod +x
echo -e "\n
${SMAY}>> Installation ${LSAMAY}Source MOD ${SMAY} Completed ^_^ .\n
Now I Have Information For Your Bot  \n
1- Enter Token Your Bot .
2- Enter Your UserName Sudo .
3- And End Press Enter To Run Source MOD .\n\n
